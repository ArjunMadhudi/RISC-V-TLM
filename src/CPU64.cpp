...
CPU64::CPU64(sc_core::sc_module_name const name, uint32_t PC) :
		sc_module(name), instr_bus("instr_bus"), default_time(10,
				sc_core::SC_NS) {
	register_bank = new Registers<uint64_t>();
	...
	register_bank->setValue(Registers<uint64_t>::sp, (0x10000000 / 4) - 1);
	...
	exec = new BASE_ISA<uint64_t>(0, register_bank, mem_intf);
	c_inst = new C_extension<uint64_t>(0, register_bank, mem_intf);
	m_inst = new M_extension<uint64_t>(0, register_bank, mem_intf);
	a_inst = new A_extension<uint64_t>(0, register_bank, mem_intf);
